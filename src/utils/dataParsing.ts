...
export function getRowsFromRecord(record: any): Row[] {
  ...
  if (Array.isArray(record.parsed_rows) && record.parsed_rows.length) {
    return record.parsed_rows.map((r: any) => ({ name: String(r.name || ""), price: String(r.price || "") }));
  }
  const maybe = extractJsonFromCodeBlock(String(record.model_raw || ""));
  if (!maybe) return [];
  const rows = Array.isArray(maybe.rows) ? maybe.rows : Array.isArray(maybe) ? maybe : [];
  return rows.map((r: any) => ({ name: String(r.name || ""), price: String(r.price || "") }));
}
